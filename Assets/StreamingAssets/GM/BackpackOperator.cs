...
namespace TempProj
{
    public class BackpackOperator : IGMOperator
    {
        private PropDataAgent m_PropDataAgent;

        public Action[] m_Actions;

        public BackpackOperator()
        {
        ...
        public string Name { get; } = "背包";
        public string[] ArgNames { get; } = {"道具Id", "道具数量"};
        public string[] Args { get; set; } = {null, null};
        public string[] ActionNames{ get; } = new [] {"设置道具数量", "添加道具数量", "减少道具数量", "清空所有道具"};
        public Action[] Actions => new Action[] {SetItemCount, AddItemCount, RemoveItemCount, RemoveAllItems};
