...
namespace TempProj
{
    ...
    public abstract class UIFormPresenter : IUIFormPresenter
    {
        ...
        public virtual void OnInit(int uniqueId, int formId, CustomUIFormLogic logic, Transform root, object userData)
        {
            UniqueId = uniqueId;
            FormId = formId;
            Root = root;
            ...
            DRUIForm drUIForm = Entry.DataTable.GetDataTable<DRUIForm>()?.GetDataRow(formId);
            if (drUIForm != null)
            {
                Entry.UI.SetUIFormInstanceLocked(logic.UIForm, drUIForm.IsLocked);
            }
        }
